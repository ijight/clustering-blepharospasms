...
def generate_partitions(weightedGraph):
    return community_louvain.best_partition(weightedGraph, resolution=(1.04))
...
def majority_partition_hodam(partition, TIMESTEP, OVERLAP):
    """
    Generate majority partition from original timeseries and generated partitions
    ...
    TRUNCATED = 0  # len(TS) % TIMESTEP
    TSFAKE = [*range(len(TS))]
    SEGMENTSFAKE = [TSFAKE[i:i + TIMESTEP] for i in range(0, len(TS) - TIMESTEP + 1,
                                                          TIMESTEP - OVERLAP)]  # (from 0 to last possible timestep, in steps of len of timestep (remove overlap))
    occuranceTable = {i: [segmentNum for segmentNum in range(len(SEGMENTSFAKE)) if i in SEGMENTSFAKE[segmentNum]] for i
                      in range(len(TS) - TRUNCATED) if any(i in seg for seg in SEGMENTSFAKE)}
